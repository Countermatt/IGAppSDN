...
class Interface():
    ...
    def __init__(self,window):
        ...
        self.sixthOnglet = ttk.Frame(self.systemOnglet,width=1500,height=1500,style='Frame4.TFrame')
        self.sixthOnglet.place(x='150',y='0')
        self.systemOnglet.add(self.sixthOnglet,text='SDN Application')
        self.canvas6 = self.create_canvas_third(self.sixthOnglet)
        self.SDNFrame = ScrolledFrame(self.canvas6)
        self.SDNFrame.pack(expand=True,fill='both')
        #self.SDNTab = self.create_SDN_tab(self.SDNFrame,"../../ryu/ryu/app/simple_switch_stp_13.py","normal")
        #self.SDNText = self.create_SDN_Text(self.SDNTab)
        #self.SDNTab =self.create_SDN_tab(self.SDNFrame,"SDN app","normal")
        self.SDNText=self.placeTextSDNApp()
    

        #self.seventhOnglet = ttk.Frame(self.systemOnglet,width=1500,height=1500,style='Frame4.TFrame')
        #self.seventhOnglet.place(x='180',y='0')
        #self.systemOnglet.add(self.seventhOnglet,text='Flow Table')
        #self.canvas7 = self.create_canvas_third(self.seventhOnglet)
        #self.FlowFrame = ScrolledFrame(self.canvas7)
        #self.FlowFrame.pack(expand=True,fill='both')
            #self.FlowTab =self.create_SDN_tab(self.FlowFrame,"Flow","normal")
            #self.FlowText=self.create_SDN_Text(self.FlowTab)
        #self.FlowText=self.placeTextFlow()
        ...
        #canvas SDN app
        self.canvas_SDN_launch=self.create_canvas_performances(self.sixthOnglet)
        self.create_button_SDN_launch(self.canvas_SDN_launch)

        #canvas Flow tables
        #self.canvas_flow=self.create_canvas_performances(self.seventhOnglet)
        #self.create_button_flow(self.canvas_flow)
    ...
    def configuration(self):
        ...
        self.SDNText.tag_configure('big',font=('Helvetica',12,'bold'),foreground='RoyalBlue1',underline=1)
        #self.FlowText.tag_configure('color',font=('Helvetica',11,'bold'))

        #self.FlowText.tag_configure('big',font=('Helvetica',12,'bold'),foreground='RoyalBlue1',underline=1)
        #self.FlowText.tag_configure('color',font=('Helvetica',11,'bold'))
    ...
    def placeTextSDNApp(self):
        helv36 = Font(family='Helvetica', size=11, weight='bold')
        labelSDNApp = Label(self.SDNFrame.inner,text='SDN Application',font=helv36,bg='slateGray1')
        labelSDNApp.pack()
        canvasSDNApp=Canvas(self.SDNFrame.inner,width=300,height=300,bg='RoyalBlue1')
        canvasSDNApp.pack()
        texte = Text(canvasSDNApp,width=120,height=30)
        scroll = Scrollbar(canvasSDNApp,command=texte.yview)
        texte.configure(yscrollcommand=scroll.set)
        texte.config(state="normal")
        texte.pack(side=LEFT)
        scroll.pack(side=RIGHT,fill=Y)
    ...
    def placeTextFlow(self):
        helv36 = Font(family='Helvetica', size=11, weight='bold')
        labelFlow = Label(self.FlowFrame.inner,text='Flow Table',font=helv36,bg='slateGray1')
        labelFlow.pack()
        canvasFlow=Canvas(self.FlowFrame.inner,width=300,height=300,bg='RoyalBlue1')
        canvasFlow.pack()
        texte = Text(canvasFlow,width=120,height=30)
        scroll = Scrollbar(canvasFlow,command=texte.yview)
        texte.configure(yscrollcommand=scroll.set)
        texte.config(state="normal")
        texte.pack(side=LEFT)
        scroll.pack(side=RIGHT,fill=Y)
    ...
    def delete_SDN_App(self):
        self.popenSDN.kill()
        self.popenSDN=None
        self.SDNText.configure(state="normal")
        self.SDNText.delete(1.0,"end")
        self.SDNText.configure(state="disabled")
        self.SDNActiveApp=False
    ...
    def create_canvas_SDN_launch(self,window):
        canvas = Canvas(window,width='120',height='1500',bg='white')
        canvas.place(x='0',y='0')
        return canvas

    def create_canvas_flow_managment(self,window):
        canvas = Canvas(window,width='120',height='1500',bg='white')
        canvas.place(x='0',y='0')
        return canvas
    ...
    def create_buttons(self,window):
        ...
        for n in self.elements :
            ...
            self.list_buttons[n]=Button(window,height=70,width=70,bg='white') #switch , cursor , host
            self.list_buttons[n].config(relief="raised")
            self.list_buttons[n].config(text=n, image=self.images[n],command=cmd, compound=TOP)
    ...
    def create_buttons_performances(self,window):
        ...
        boutonserver = Button(window,width=9,height=2,bg='white',text='Iperf Server',font=helv36,command=self.iperfserver)
    ...
    def create_button_SDN_launch(self, window):
        helv36 = Font(family='Helvetica', size=10, weight='bold')
        boutonLink=Button(window,width=12,height=3,bg='white',text='Launch SDN\nApplication',font=helv36,command=self.loadSDNApp)
        boutonLink.place(x='5',y='150')
        boutonLink=Button(window,width=12,height=3,bg='white',text='Clean up',font=helv36,command=self.delete_SDN_App)
        boutonLink.place(x='5',y='250')

    def create_button_flow(self, window):
        helv36 = Font(family='Helvetica', size=6, weight='bold')
        boutonLink=Button(window,width=10,height=3,bg='white',text='Display \nFlow Table',font=helv36)
        boutonLink.place(x='5',y='150')
        boutonLink=Button(window,width=10,height=3,bg='white',text='Add Flow',font=helv36)
        boutonLink.place(x='5',y='250')
    ...
    def show(self, outputText, message):
        """Inserts message into the Text wiget"""
        #self.bar_coordinates = self.SDNText.vbar.get()
        outputText.config(state="normal")
        msg = "".join(message)
        outputText.insert("end", msg)
        #self.SDNText.see("end")
        #self.SDNText.vbar.set(self.bar_coordinates[0],self.bar_coordinates[1])
    ...
    def stop_thread(self):
        ...
        if self.popenSDN:
            try:
                ...
                #print("Erreur dans stop_thread")
                pass 
        self.running = False



    def startSDNFile(self, filepath, event=None):
        self.stop_thread()
        self.running = True
        # self.process is called by the Thread's run method
        self.redirectSDN(filepath, self.SDNText)



    def showSDNfile(self, SDNText,startline=0):
        SDNText.config(state="normal")
        #self.SDNText.delete(1.0, END)
        SDNText.config(state="disabled")
        filename = "SDNOutput1.txt"
        with open(filename) as f2:
            read_data = f2.readlines()
            if read_data:
                if startline == 0:
                    self.show(SDNText,read_data)
                    startline = startline + len(read_data)
                elif startline < len(read_data):
                    read_data = read_data[startline:]
                    self.show(SDNText,read_data)
                    startline = startline + len(read_data)
        if self.SDNActiveApp == True: 
            self.SDNFrame.after(1, self.showSDNfile, SDNText, startline)





    def showStaticOutput(self, outputFile, TextFrame):
        TextFrame.delete(1.0, END)
        with open(outputFile) as f2:
            read_data = f2.read()
            self.show(TextFrame,read_data)

        
    def redirectSDN(self, filepath, SDNText):
        """Keeps inserting line by line into self.SDNTexts
        the output of the execution of self.command"""
        filename = "SDNOutput1txt"
        f = open(filename,"w")
        f.close()
        self.command="ryu-manager " + filepath
        print(self.command)
        self.SDNActiveApp=True
        with open(filename,"w") as f:
            self.popenSDN = subprocess.Popen(self.command, stdout=f, stderr=f, shell=True, bufsize=1)
        self.SDNFrame.after(1,self.showSDNfile, SDNText)


    def redirectStaticCommand(self,command, outputfile, TextFrame):
        """Inserts into self.FirewallRulesText
        the output of the execution of self.command"""
        f = open(outputfile,"w")
        f.close()
        ...
        with open(outputfile,"w") as f:
            self.popenFlow = subprocess.Popen(self.command, stdout=f, stderr=f, shell=True, bufsize=1)
        self.FlowFrame.after(1,self.showStaticOutput, outputfile, TextFrame)
	...
	#self.processFirewall.terminate()
    ...
    def netImages(self):
        img1=PhotoImage(file="./graph/switch.gif")
        img2=PhotoImage(file="./graph/controleur.png")
        img3=PhotoImage(file="./graph/host.png")
        img4=PhotoImage(file="./graph/line.png")
        img5=PhotoImage(file="./graph/legacyRouter.png")
        img6=PhotoImage(file="./graph/legacyswitch.png")
